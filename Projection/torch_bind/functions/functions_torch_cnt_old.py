...
def CrossVal(X, Y, patient_name, BATCH_SIZE=32, nfold=0, seed=1):
    ...
    #     #ind_train=np.concatenate((ind_train, ind_test))
...
def RunAutoEncoder(
    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        for i, batch in enumerate(train_dl):
            ...
            labels = batch[1]
            #print(labels)
            labelArr=np.array(labels.tolist())
            #print(labelArr.shape)
            distRun=sc.gaussian_kde(labelArr, bw_method=0.2)
            distTG=torch.Tensor(distRun.evaluate(labelArr))
            ...
            labelPred=np.array(torch.t(encoder_out).tolist()[0])
            Y_predit.extend(labelPred)
            #print(labelPred)
            distPred=sc.gaussian_kde(labelPred, bw_method=0.2)
            distPG=torch.Tensor(distPred.evaluate(labelPred))
            # print(labels)
            # print(labelPred)
            # fig = plt.figure()
            # ax = fig.add_subplot(111)
            x_eval = np.linspace(0, 1, num=len(labelPred))
            # ax.plot(x_eval, gaussianKDE(x_eval), label="true",color="tab:blue" )
            # ax.plot(x_eval, distRun(x_eval), label="this round",color="tab:pink" )
            # ax.plot(x_eval, distPred(x_eval), label="Pred",color="tab:green" )
            # plt.xlabel('Weight')
            # plt.ylabel('Density')
            # plt.title('Weight Distribution between predicted and the truth during each fold')
            # plt.legend(["true","of this round","Predicted"])
            # plt.show()
            # print(distPG)
            # print( distPred(x_eval))
            #print(distPG)
            # print(encoder_out)
            # print(encoder_out.log())
            # print(torch.nn.functional.log_softmax(encoder_out,dim=0))
            loss_classification = criterion_classification(torch.Tensor(distPred(x_eval)), torch.Tensor(gaussianKDE(x_eval)))
            print(loss_classification)
            #loss_classification = criterion_classification(encoder_out, labels)
            #with torch.no_grad():
            # Compute the loss
            
                #loss_classification= mean_squared_error(labels.long(), encoder_out)
            ...
            with torch.no_grad():
                ...
                running_mse += mean_squared_error(labels.long(), encoder_out.max(1)[1])
        ...
        epoch_acc.append(running_mse / train_len)
...
# =============================================================================
        ...
        #         encoder_out, decoder_out = net(x)
        #         # Compute the loss
        #         loss_classification = criterion_classification(encoder_out.squeeze(-1), labels)
        #         # with torch.no_grad():
        ...
        #         running_reconstruction += loss_reconstruction.item()
        #         running_classification += loss_classification.item()
        #         running_mse += mean_squared_error(labels.long(), encoder_out)
        ...
        # if running_mse < best_test:
        #     best_net_it = epoch_idx
        #     best_test = running_mse
        ...
        # epoch_val_acc.append(running_mse / test_len)
    ...
    plt.xlabel('Weight')
    plt.ylabel('Density')
    plt.title('Weight Distribution between predicted and the truth during TRAINING')
...
def runBestNet(
    ...
    for i, batch in enumerate(train_dl):
        ...
        # print(torch.nn.functional.log_softmax(encoder_out,dim=0))
        mean_squared_error(
            encoder_out.ttensor.detach().numpy(), labels
        )
    ...
    for i, batch in enumerate(test_dl):
        with torch.no_grad():
            ...
            #print(encoder_out)
            Y_predit.append(encoder_out.max(1)[0].item())
            Y_true.append(labels.item())
            #c = (encoder_out.max(1)[0] == labels).squeeze()
            # for i in range(len(x)):
            #     label = int(labels[i].item())
            #     if c.dim() == 0:
...
def topGenes(
    ...
    input_y = torch.from_numpy(Y).float().to(device)
    if method == "Shap":
    ...
    dl_attr_test_sum = attributions.detach().numpy().sum(0)
    dl_attr_test_norm_sum = dl_attr_test_sum / np.linalg.norm(dl_attr_test_sum, ord=1)
    indices = [i for i,v in enumerate(dl_attr_test_norm_sum) if v > 0.000]
    x_axis_labels=[x_axis_data_labels[i] for i in indices]
    dl_attr= [dl_attr_test_norm_sum[i] for i in indices]
    x_axis_data_plt=x_axis_data[1:len(dl_attr)+1]
    print(x_axis_labels)
    print(x_axis_data_plt)
    print(dl_attr)
    width = 0.8
    ...
    ax.bar(x_axis_data_plt , dl_attr, width, align='center', alpha=0.6, color='#34b8e0')
    ax.autoscale_view()
    plt.tight_layout()
    
    ax.set_xticks(x_axis_data_plt)
    ax.set_xticklabels(x_axis_labels)
    plt.xticks(rotation=90)
    plt.show()
    
    feature_rank[:, 0] = x_axis_data_labels
    feature_rank[:,1] = dl_attr_test_norm_sum
...
def ReadData(
    ...
    Y = data_pd.iloc[0, 1:].values.astype(float)
    ...
    for i in range(len(Y)):
        newY[i]=(Y[i]-minY)/coef
    
    # if not Y.all():
    #     Y += 1  # 0,1,2,3.... labels -> 1,2,3,4... labels
    newY=np.array(newY)
    gaussianKDE=sc.gaussian_kde(newY, bw_method=0.2)
    
    return X, newY, feature_name, label_name, patient_name, gaussianKDE
