...
def bilevel_proj_l1Inftyball_parallel(w2, eta, device="cpu", nb_cores = 4):
     ...
     if w.dim() == 1:
         ...
         with Pool(nb_cores) as p:
             ...
             W = torch.tensor(p.starmap(f1, zip([i for i in
range(ncol)], [w for _ in range(ncol)])))

             # W = torch.tensor(
             #     [torch.max(torch.abs(w[:, i])).data.item() for i in
#range(ncol)]
             # )
             ...
             W = p.starmap(f2, zip([i for i in range(ncol)], [w for _ in
range(ncol)], [PW for _ in range(ncol)]))

             # for i in range(ncol):
             #     Res[:, i] = torch.clamp(torch.abs(w[:, i]),
#max=PW[i].data.item())
             #     Res[:, i] = Res[:, i] * torch.sign(w[:, i])
             for i in range(ncol):
                 Res[:, i] = W[i]
...
def Projection(
    ...
    if TYPE_PROJ == bilevel_proj_l1Inftyball_parallel:
        W_new = TYPE_PROJ(W, ETA, device="cpu",nb_cores = 1)      
