    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        for i, batch in enumerate(train_dl):
            ...
            with torch.no_grad():
                ...
                running_mse += mean_squared_error(labels, encoder_out.max(1)[1])
...
# =============================================================================
        ...
        for i, batch in enumerate(test_dl):
            with torch.no_grad():
                ...
                running_reconstruction += loss_reconstruction.item()
                running_classification += loss_classification.item()
                running_mse += mean_squared_error(labels, encoder_out)
        """
        print(
            "test accuracy : ",
...
def runBestNet(
    ...
    for i, batch in enumerate(test_dl):
        with torch.no_grad():
            ...
            # print(encoder_out.max(1))
            # print(encoder_out.max(1)[0].item())
            # print(encoder_out.max(1)[0])
            # print(encoder_out.max)
...
def ReadData(
    ...
    return X, newY, feature_name, label_name, patient_name
