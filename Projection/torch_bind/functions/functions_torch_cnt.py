...
def RunAutoEncoder(
    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        for i, batch in enumerate(train_dl):
            ...
            loss_classification = criterion_classification(encoder_out.squeeze(), labels)
            #with torch.no_grad():
            # Compute the loss
            ...
            with torch.no_grad():
                ...
                running_mse +=loss_classification.item()
        ...
        for i, batch in enumerate(test_dl):
            with torch.no_grad():
                ...
                if torch.cuda.is_available():
                    x = x.cuda()
                    labels = labels.cuda()
                encoder_out_test, decoder_out = net(x)
                Y_predit.extend(encoder_out.detach().numpy().flatten())
                Y_true.extend(labels.detach().numpy().flatten())
                # Compute the loss
                #loss_classification = criterion_classification(torch.nn.functional.log_softmax(encoder_out,dim=1), labels)
                loss_classification = criterion_classification(encoder_out_test.flatten(), labels)
                # with torch.no_grad():
...
def runBestNet(
    ...
    try:
        if nfold != 0:
            df = pd.read_csv(
                "{}Labelspred_weight.csv".format(outputPath), sep=";", header=0
            )

            soft = pd.DataFrame(index_pred_probs)
            soft = pd.DataFrame(
                np.concatenate((df.values, soft.values[:, :])),
                columns=["Name", "Initial Weight"]
                + ["Predicted weight"],
            )
            soft.to_csv("{}Labelspred_weight.csv".format(outputPath), sep=";", index=0, float_format='%g')
        else:
            soft = pd.DataFrame(
                index_pred_probs,
                columns=["Name", "Initial Weight"]
...
def valueGap (true, predicted):
    ...
    return resultNeg/len(true), resultPos/len(true)
...
def PSNR( true, predicted):
    ...
    denom = sum([element[0] for element in true.tolist()])/len(predicted.tolist())**2
...
def packMetricsResult(data_train, data_test, fold_nb):
    ...
    """
    columns = (
        ["MSE"]+ ["MSE std"]+["Negative gap"]+ ["Positive gap"] +["PSNR"]
    )
    ind_df = ["Fold " + str(x + 1) for x in range(fold_nb)]
...
def topGenes(
    ...
    # data=list(zip(x_axis_labels,dl_attr))
    ...
    # ax.bar(x_axis_data_plt , sorted_value, width, align='center', alpha=0.6, color='#34b8e0')
    # ax.autoscale_view()
    # plt.tight_layout()
...
def ReadData(
    ...
    return X, Y, feature_name, label_name, patient_name, gaussianKDE
