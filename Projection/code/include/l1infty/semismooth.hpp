/*
 * @author Ariadna Quattoni (original Algorithm)
 * @author Guillaume Perez (C++ translation)
 *
 * @brief This code is an implementation of 
 * Ariadna Quattoni, Xavier Carreras, Michael Collins, and Trevor
 * Darrell. An Efficient Projection for L1,Infinity Regularization, 
 ...
 */
inline void fmutheta(double Fval[], double grad_mu[], double B[],
...
inline void print_Data(double *m, int nRows,int nCols) {
    int i,j,index;
    double value;
...
inline double norm_l1infinity(double *m, int nRows, int nCols) {
    double norm=0;
    int i, j, index;
    for(i=0;i<nRows;i++) {
        double maxRow=0;
 ...
 */
inline void mySemismoothNewton(double B[], double C, double A[],
                        int nRows, int nCols){
...
inline void DJCHU(double *y, double *x, const int nrows, const int ncols,
                     const double C) {
  djchu::mySemismoothNewton(x, C, y,  nrows, ncols);
}
