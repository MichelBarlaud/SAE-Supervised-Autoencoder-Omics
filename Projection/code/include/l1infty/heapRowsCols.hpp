...
inline void HeapRowsCols(double* y, double* x, const int nrows,
  ...
  std::make_heap(Res, Res + ncols_p1 * nrows);

  double theta = (theta_num - C) / theta_den;
  bool changed = true;
  ValueCoord *end = Res + ncols_p1 * nrows;
  for (size_t i = 0; i < nrows; i++) {
  ...
  while (end != Res) {
    // std::cout << theta << " ";
    int i = (*Res).coordinates / ncols_p1;
    int j = (*Res).coordinates % ncols_p1;
    size_t id = i * ncols + j;
    // std::cout << i << "\n ";
