...
def proj_l11ball(w2, eta, device="cpu"):
    ...
    if w.dim() == 1:
        ...
        for i in range(ncol):
            Res[:, i] = proj_l1ball(w[:, i], eta, device=device)
...
def normalisationEta(TYPE_PROJ, X):
    if (
        TYPE_PROJ == proj_l1ball
        or TYPE_PROJ == bilevel_proj_l11ball
        or TYPE_PROJ == proj_l11ball
    ):
        return LA.norm(X, 1)
    if TYPE_PROJ == proj_l21ball:
        print("No norm applied with proj_l21ball")
        return X
    if (TYPE_PROJ == proj_l1infball 
        or TYPE_PROJ == bilevel_proj_l1Inftyball
        or TYPE_PROJ == proj_l1Inftyball_line_unbounded) :
        return norml1inf(X)
...
class FairAutoEncoder(torch.nn.Module):
    ...
    Attributes:
        ...
        activation: str - Activitation function to use
        norm: Bool - Put Layer Norm between each layer
                ...
                if(( DO_PROJ_MIDDLE == True or not is_middle ) and ( not is_decoder_layer or DO_PROJ_DECODER )):
                    if is_decoder_layer:
                        param.data = Projection(
                            param.data.T, TYPE_PROJ, ETA, AXIS=AXIS, ETA_STAR=ETA_STAR, device=device, TOL=TOL,).to(device).T
                    else:
                        param.data = Projection(
